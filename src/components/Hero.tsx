    ...
    <>
      ...
      >
        ...
        <div className="flex flex-row pt-8 space-x-2 sm:space-x-5 md:space-x-10">
          <button onClick={() => handleClick('aboutSection')} className="bg-white/80 text-black px-3 sm:px-5 py-2 rounded-full text-[min(3vw,16px)] font-bold hover:bg-white cursor-pointer transform hover:scale-[1.15] transition duration-200 ease-in-out">Learn More</button>
        
          <button onClick={() => handleClick('projectsSection')} className="bg-sky-950/90 border-2 border-white/80 text-white px-3 sm:px-5 py-2 rounded-full text-[min(3vw,16px)] font-bold hover:bg-white hover:text-black cursor-pointer transform hover:scale-[1.15] transition duration-200 ease-in-out">View Featured Projects</button>
        </div>
      </motion.div>
    </>
  );
